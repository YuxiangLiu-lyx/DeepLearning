...
class Softmax(Module):
    ...
    def forward(self: Softmax,
        ...
        X = np.atleast_2d(X)
        X_stretched = self.stretch * X
        X_max = np.max(X_stretched, axis=-1, keepdims=True)
        X_shifted = X_stretched - X_max
    ...
    def jacobian_single_example(self: Softmax,
        ...
        n = y_hat.shape[0]
        diag_y = np.diag(y_hat)
        outer_y = np.outer(y_hat, y_hat)
        jacobian = self.stretch * (diag_y - outer_y)
        return jacobian
